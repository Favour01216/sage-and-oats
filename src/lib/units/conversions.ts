 ...
 */
export function formatQuantity(value: number, unit: string, system: UnitSystem): string {
  // Use fractions for US measurements
  const useFractions = system === "us";
  const formatted = useFractions
    ? formatMixedFraction(value)
...
export function processQuantity(
  ...
  const { value: converted, unit: targetUnit } = convertToSystem(
    scaled,
    unit,
    targetSystem,
    ingredient,
