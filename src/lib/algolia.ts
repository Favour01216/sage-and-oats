...
export const ALGOLIA_INDEX = process.env.NEXT_PUBLIC_ALGOLIA_INDEX_NAME || "recipes";

// IMPORTANT: use a SEARCH-ONLY key in the browser, never the admin key.
export const ALGOLIA_SEARCH_KEY = process.env.NEXT_PUBLIC_ALGOLIA_SEARCH_KEY ?? ""; // add this env if missing
...
export async function ensureIndexSettings() {
  if (!adminClient) {
    console.warn("Admin client not available (client-side or missing admin key)");
    return;
  }
