...
export function useSearchWithDebounce(
  ...
  const fetchData = useCallback(async (searchParams: SearchParams, signal: AbortSignal) => {
    try {
      ...
      const response = await fetch(
        `/api/catalog/search?${urlParams.toString()}`,
        { signal }
      );
  ...
  }, [params.q, params.page, params.perPage, ...(params.tags || []), ...(params.cuisine || []), params.time?.min, params.time?.max, params.calories?.min, params.calories?.max, debouncedSearch]);
