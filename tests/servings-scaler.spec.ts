...
test.describe("Servings Scaler", () => {
  test("doubles quantities when scaling from 2 to 4 servings", async ({ page }) => {
    ...
    if ((await servingsControl.count()) > 0) {
      ...
      const initialServings = await page
        .locator("text=Servings:")
        .locator("..")
        .locator("span")
        .nth(1)
        .textContent();
  ...
  test("persists serving selection per recipe", async ({ page }) => {
    ...
    if ((await servingsControl.count()) > 0) {
      ...
      // Increase servings
      await increaseButton.click();
      const servingsAfterIncrease = await page
        .locator("text=Servings:")
        .locator("..")
        .locator("span")
        .nth(1)
      ...
      const servingsAfterReturn = await page
        .locator("text=Servings:")
        .locator("..")
        .locator("span")
        .nth(1)
